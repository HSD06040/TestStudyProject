...
public class AudioManager : Singleton<AudioManager>, ISavable
{
    ...
    private void Awake()
    {
        ...
        audioMixer = RM.Load<AudioMixer>($"{SOUND_PATH}AudioMixer");
    ...
    public void PlaySFX(string name, Vector3 position, float volume = 1, float pitch = 1)
    {
        ...
        GameObject audioObj = RM.Instantiate<GameObject>($"{SOUND_PATH}SFX_Obj", position, true);
        ...
        audio.Play();
        RM.Destroy(audioObj, clip.length / pitch);
    }
    ...
    private AudioClip LoadClip(string name, SoundType type)
    {
        ...
        if(type == SoundType.SFX)
        {
            ...
            AudioClip clip = RM.Load<AudioClip>($"{SOUND_PATH}{type}/{name}", true);
        ...
        return RM.Load<AudioClip>($"{SOUND_PATH}{type}/{name}");
    }
    ...
    {
        if (type == SoundType.SFX)
            audioMixer.SetFloat("SFX", SliderValueToDecibel(volume));
        else
            audioMixer.SetFloat("BGM", SliderValueToDecibel(volume));
    }
    public float GetVolume(SoundType type)
    {
        float db;
        audioMixer.GetFloat(type.ToString(), out db);
        db = DecibelToSliderValue(db);
