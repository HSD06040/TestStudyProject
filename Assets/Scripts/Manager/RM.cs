...
public static class RM
{
    ...
    public static T Load<T>(string path, bool isCached = true) where T : Object
    {
    ...
    public static T Instantiate<T>(string path, Vector3 position, Quaternion rotation, Transform parent, bool isPool = false) where T : Object
    {
        T obj = Load<T>(path);
        return Instantiate(obj, position, rotation, parent, isPool);
    }

    public static T Instantiate<T>(string path, Vector3 position, Quaternion rotation, bool isPool = false) where T : Object
    {
        return Instantiate<T>(path, position, rotation, null, isPool);
    }

    public static T Instantiate<T>(string path, Vector3 postion, bool isPool = false) where T : Object
    {
    ...
    public static void Destroy(GameObject obj, float delay)
    {
        if (Manager.Pool.ContainsKey(obj.name))
